...
begin
  ...
  if @options[:endpoint].empty? || @options[:user].empty? || @options[:password].empty?
    puts 'Argument error: Options "--user", "--password" and "--endpoint" must be supplied!'
    exit NAGIOS_UNKNOWN
  end
  ...
  puts "Foreman: Unable to connect to foreman instance #{@options[:endpoint]}"
  exit NAGIOS_UNKNOWN
rescue StandardError => e
  puts "Foreman: Unknown error: #{e.message}"
  exit NAGIOS_UNKNOWN
end
